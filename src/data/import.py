...
if __name__ == "__main__":
    ...
    #main( "medeo" ,f"https://api.openweathermap.org/data/2.5/weather?lat=48.8566&lon=2.3522&appid={os.getenv("METEO_KEY")}" )
        ...
        print(f"☀️    {datetime.now(timezone.utc)} - Appel API meteo ")
    ...
    if mode == "minute":
        print(f"📞    {datetime.now(timezone.utc)} - Appel de la fonction transformation Mongo vers postgreSQL") 
        transform.main()
        print(f"⏰    {datetime.now(timezone.utc)} - Fin de l'appel à la fonction tranformation") 
