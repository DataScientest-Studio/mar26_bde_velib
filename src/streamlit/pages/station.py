...
try:
    with st.spinner("Chargement des données..."):
        ...
        hours = "".join([f"&heure={i:02d}:00" for i in range(x.hour, 24)])
        ...
        #print(f"http://{HOST}:8000/v1/predictions/station?id_station={id_station}{hours}")
    # ══════════════════════════════════════════════
    ...
    if r_semaine.status_code == 200:
        ...
        df_sem = df_sem.fillna(0)
        print(f" df_sem : {df_sem}")
    ...
    if r_pred.status_code == 200:
        ...
        # Extraire la liste de prédictions imbriquée
        if isinstance(data_pred, list):
            predictions_list = data_pred[0]["predictions"]
        elif "predictions" in data_pred:
            predictions_list = data_pred["predictions"]
        else:
            predictions_list = [data_pred]
