...
FAKE_STATION = Station(  id_station=1,   nom_station="République",   longitude=2.36,   latitude=48.86,   capacite_totale=20,    proximite=[],)

FAKE_STATION_2 = Station( id_station=2,  nom_station="Bastille",   longitude=2.37,   latitude=48.85,  capacite_totale=15,  proximite=[],)

FAKE_ETAT = EtatStation( id_station=1,nb_velo=10,nb_velo_classique=7,nb_velo_electrique=3,nb_place_libre=10,capacite_totale=20,
                        derniere_maj=datetime(2025, 1, 15, 8, 0, 0),  meteo=Meteo(description="Ensoleillé", temperature=12.0, humidite=60.0, vent=15.0),)

FAKE_ETAT_2 = EtatStation(id_station=2,nb_velo=5, nb_velo_classique=3,nb_velo_electrique=2,nb_place_libre=10,capacite_totale=15,
                          derniere_maj=datetime(2025, 1, 15, 8, 0, 0), meteo=Meteo(description="Nuageux", temperature=10.0, humidite=70.0, vent=20.0),)


FAKE_STATS_GLOBAL = StatsGlobal(nb_stations_total=100,nb_stations_actives=90,nb_velos_disponibles=500,nb_places_libres=300,
...
def client():
    ...
    with \
        ...
        patch("src.api.services.predictions_service.predict_metro",
              return_value=FAKE_PREDICTION_METRO), \
