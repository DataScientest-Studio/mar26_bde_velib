...
class TestPredictionStation:
    def test_returns_200(self, client):
        response = client.get("/v1/predictions/station?id_station=1057387136&heure=08:30")
        assert response.status_code == status.HTTP_200_OK

    def test_structure(self, client):
        response = client.get("/v1/predictions/station?id_station=1057387136&heure=08:30")
        data = response.json()
        assert data["id_station"] == 1057387136
    ...
    def test_with_explicit_date(self, client):
        response = client.get(
            "/v1/predictions/station?id_station=1057387136&heure=08:30&date=2025-01-15"
        )
        assert response.json()["date"] == "2025-01-15"

    def test_invalid_heure_format(self, client):
        response = client.get("/v1/predictions/station?id_station=1057387136&heure=8h30")
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY

    def test_missing_required_param(self, client):
        response = client.get("/v1/predictions/station?id_station=1057387136")
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
class TestPredictionTrajet:
    def test_with_station_ids(self, client):
        response = client.get(
            ...
            "?id_station_depart=1057387136&id_station_arrivee=213680917&heure_depart=08:30"
        )
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert data["station_depart"]["id_station"] == 1057387136
        assert data["station_arrivee"]["id_station"] == 213680917

    def test_with_gps(self, client):
        response = client.get(
            "/v1/predictions/trajet"
            "?lat_depart=48.784567029674&lon_depart=2.3979035764933"
            "&lat_arrivee=48.827673823166&lon_arrivee=2.3209181427956"
            "&heure_depart=08:30"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_missing_depart_info(self, client):
        response = client.get(
            "/v1/predictions/trajet?id_station_arrivee=213680917&heure_depart=08:30"
        )
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
...
class TestPredictionsAuth:
    def test_station_requires_auth(self, unauthenticated_client):
        response = unauthenticated_client.get("/v1/predictions/station?id_station=1057387136&heure=08:30")
    ...
    def test_trajet_requires_auth(self, unauthenticated_client):
        response = unauthenticated_client.get(
            "/v1/predictions/trajet?id_station_depart=1057387136&id_station_arrivee=213680917&heure_depart=08:30"
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
